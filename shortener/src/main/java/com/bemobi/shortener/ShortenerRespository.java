...
public class ShortenerRespository {
    ...
    private static final String FIND_ORIGINAL_URL = "SELECT top 1 * FROM SHORT_URL WHERE SHORTEN_URL= ?";
    ...
    public Url findOriginalUrl(String shortUrl) {
        try {
            return jdbcTemplate.queryForObject(FIND_ORIGINAL_URL, new String[]{shortUrl}, new ShortUrlRowMapper());
        } catch (EmptyResultDataAccessException e) {
            return null;
        }
    }
